...
const OutputDisplay: React.FC<OutputDisplayProps> = ({ content, isGenerating, displayedContent, scriptId }) => {
  const [copiedType, setCopiedType] = useState<string | null>(null);
  const showSkeleton = isGenerating && !displayedContent;

  const trackInteraction = async (type: "copy" | "markdown" | "download") => {
    if (!scriptId) return;
    try {
      const body: any = {};
      if (type === "copy") body.clicked_copy = true;
      if (type === "markdown") body.clicked_markdown = true;
      if (type === "download") body.clicked_download = true;
      ...
      console.log(`Tracked interaction: ${type} for script ${scriptId}`);
    } catch (err) {
      console.error("Failed to track interaction:", err);
    }
  };

  // Helper to convert Markdown table to TSV for Excel
  const markdownToTsv = (md: string): string => {
    return md.split('\n').map(line => {
      const trimmed = line.trim();
      if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
        // Skip separator rows |---|---|
        if (trimmed.match(/^\|[\s\-:|]+\|$/)) return null;
        // Split by pipe, trim whitespace, join with tab
        return trimmed.split('|').slice(1, -1).map(c => c.trim()).join('\t');
      }
      return line;
  ...
  const handleCopy = async () => {
    try {
      ...
      // Add border attribute for Excel to recognize table structure
      html = html.replace('<table class="script-table">', '<table border="1" class="script-table">');

      // Wrap in full HTML document for context
      const fullHtml = `<html>
<head>
...
${html}
      ...
      const tsv = markdownToTsv(content); // Use TSV for plain text paste (Excel friendly)
      ...
      // Fallback: write TSV as plain text
      console.warn("ClipboardItem failed, falling back to basic copy", err);
      await navigator.clipboard.writeText(markdownToTsv(content));
    }

    setCopiedType("copy");
    trackInteraction("copy");
    toast.success("Copied Table (Excel Ready)!");
...
<style>body{font-family:'Li Ador Noirrit',sans-serif;font-size:14px;line-height:1.6}table{border-collapse:collapse;width:100%}td,th{border:1px solid #ccc;padding:8px;text-align:left}th{background:#f4f4f4;font-weight:600}h1{font-size:20px}h2{font-size:16px}</style></head>
      ...
      {content && !isGenerating && (
        <div className="absolute top-4 right-4 flex gap-1.5">
          {[
            ...
            { action: handleDownloadWord, type: "download", icon: Download, label: "Download .doc File" },
